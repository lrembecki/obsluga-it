...
## Configure via DI
You can configure `features` and `columns` via Angular DI tokens so parent routes/components can centrally define table behavior without passing inputs.

### Tokens
- `DataTableSchemaScope`: `TableFeatures` configuration.
- `DataTableColumnSchemaScope`: `TableColumnConfig<T>[]` column schema.

### Provide in a Route or Component
```ts
import { provideRouter, Routes } from '@angular/router';
import { DataTable } from 'app/shared/ui/data-table/data-table';
import { DataTableSchemaScope, DataTableColumnSchemaScope } from 'app/shared/ui/data-table/data-table';
    ...
    providers: [
      {
        ...
        useValue: { quicksearch: true, reorderable: true, editable: true },
      },
      {
        provide: DataTableColumnSchemaScope,
        useValue: [
...
export class ExampleTableHost {
  constructor(public readonly facade: AnyFacade) {}
  onSave(row: any) { /* facade.update(row.id, row) */ }
  onOrderBy(sort: { column: string; direction: 'asc' | 'desc' }) { /* handle */ }
}
```

Notes:
- When provided, DI `columns` override the `[columns]` input; DI `features` merge over the input (DI wins on conflicts).
- Column reordering persistence still applies and emits `columnsChange` when DI columns are used at init.
