      ...
      <tbody>
        @for (row of visibleData(); track row[rowIdField()]) {
          <tr>
            ...
            @if (isEditing(row) || features().editable) {
              <td class="actions">
                @if (isEditing(row)) {
                  <app-button
                    [text]="saveLabel()"
...
export class DataTable<T extends Record<string, any>> {
    ...
    columns = input.required<TableColumnConfig<T>[]>();
    features = input<TableFeatures>({} as TableFeatures);
    ...
    persistenceKey = input<string>('default');
    isEditable = input<boolean>(false);

    // Outputs
    save = output<T>();
    orderBy = output<TableSort>();
    columnsChange = output<TableColumnConfig<T>[]>();
    ...
    protected readonly orderedColumns = signal<TableColumnConfig<T>[]>([]);
    protected dragSourceIndex = -1;

    protected readonly normalizedColumns = computed(() => this.columns());

    ngOnInit() {
        this.orderedColumns.set(this.normalizedColumns());
    }
    ...
    protected getSortIndicator(col: TableColumnConfig<T>): string {
        const s = this.sort();
        if (!s || s.column !== col.field) return '';
        return s.direction === 'asc' ? '↑' : '↓';
    }

    protected toggleSort(col: TableColumnConfig<T>) {
    ...
    protected persistColumnOrder(cols: TableColumnConfig<T>[]) {
        try {
    ...
    ngAfterViewInit() {
        try {
            ...
            const current = [...this.normalizedColumns()];
            current.sort((a, b) => {
    ...
    protected renderCell(row: T, col: TableColumnConfig<T>) {
    ...
    protected getEditValue(col: TableColumnConfig<T>): string | undefined {
        const draft = this.editDraft();
        if (!draft) return undefined;
        return String((draft as any)[col.field] ?? '');
    }

    protected setEditValue(col: TableColumnConfig<T>, value: string | undefined) {
        const draft = this.editDraft();
        if (!draft) return;
