...
- This chat mode always uses `angular-cli/get_best_practices` and `angular-cli/search_documentation` tools to ensure adherence to the latest Angular best practices and official documentation.
- This chat mode must remain fully consistent with the canonical Angular Guidelines file. If any conflicts arise, it must defer to that file and surface the discrepancy explicitly.
- This chat mode uses `Microsoft Docs/microsoft_docs_search` and `Microsoft Docs/microsoft_code_sample_search` tools to reference official Microsoft documentation and code samples when relevant.
